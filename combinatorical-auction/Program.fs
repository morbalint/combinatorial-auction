...
let printCcaResult (bids: List<float * Bid>) =
    ...
    |> List.map (fun (accept, bid) ->
        (bid.route.player.id, accept, (accept * bid.quantity), bid.quantity, (routePrinter " -> " bid.route.edges)))
    |> List.map (fun (playerId, accept, assignedCapacity, requestedCapacity, route) ->
        sprintf
            "bid of player %i was accepted at rate %.3f, assigned capacity: %.1f, requested capacity: %.1f, route: %s"
            playerId
            accept
            assignedCapacity
            requestedCapacity
...
let twistBid twister (bids: Bid list) index =
    ...
    if index = 0 then
        ...
        List.concat [ bids.GetSlice(Some 0, Some(index - 1))
                      [ twistedBid ]
                      bids.GetSlice(Some(index + 1), None) ]

let replaceBidInResult ccaResult originalBid =
    ccaResult
    |> List.map (fun (a, bid: Bid) ->
        if bid.route = originalBid.route
           && bid.quantity = originalBid.quantity then
            (a, originalBid)
        else
            (a, bid))
...
let tester (bids: Bid list) (originalProfits: System.Collections.Generic.IDictionary<Player, float>) rate =
    ...
    |> Seq.mapi (fun index tbids ->
        ...
        let originalBids =
            replaceBidInResult vcgRes.acceptance bids.[index]

        let profits =
            payments.Keys
            |> Seq.map (fun player ->
                (player,
                 ((playerValue originalBids player)
                  - payments.[player])))
...
let ACAtester () =
    ...
    let rec loop already_closed_edges total_increment bids previous_results =
        ...
        let results =
            Seq.append previous_results partial_results
